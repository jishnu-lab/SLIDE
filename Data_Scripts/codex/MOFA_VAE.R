...
x <- read.table("/ix/djishnu/Javad/SLIDEbench_Codex/x.csv",
                row.names = 1,
                sep=",",
                header = T)

y <- read.table("/ix/djishnu/Javad/SLIDEbench_Codex/y.csv",
                row.names = 1,
                sep = ",",
                header=T)
...
Z_ER <- read.table("/ix/djishnu/Javad/SLIDEbench_Codex/z_matrix.csv",header = T,
                   row.names = 1,sep = ",")
...
saveRDS(vaeoadings,"/ix/djishnu/Javad/SLIDEbench_Codex/MOFA_VAE/VAE_loadings.rds")
saveRDS(encoder_output[,1:9],"/ix/djishnu/Javad/SLIDEbench_Codex/MOFA_VAE/VAE_factors.rds")
