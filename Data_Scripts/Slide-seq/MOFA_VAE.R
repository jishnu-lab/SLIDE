...
x <- read.table("/ix/djishnu/Javad/SLIDE_slide-seq/x.csv",
                row.names = 1,
                sep=",",
                header = T)

y <- read.table("/ix/djishnu/Javad/SLIDE_slide-seq/y.csv",
                row.names = 1,
                sep = ",",
                header=T)
...
Z_ER <- read.table("/ix/djishnu/Javad/SLIDE_slide-seq/z_matrix.csv",header = T,
                   row.names = 1,sep = ",")
...
vae_fit <- vae %>% fit(data_norm, data_norm, epochs = 20, batch_size = 24, validation_split = 0.2)
encoder_output <- stats::predict(encoder, data_norm)
row.names(encoder_output) <- row.names(x)
...
saveRDS(vaeoadings,"/ix/djishnu/Javad/SLIDE_slide-seq/MOFA_VAE/VAE_loadings.rds")
saveRDS(encoder_output[,1:9],"/ix/djishnu/Javad/SLIDE_slide-seq/MOFA_VAE/VAE_factors.rds")
...
saveRDS(factors,"/ix/djishnu/Javad/SLIDEbench_Codex/MOFA_VAE/MOFA_facros_v2.rds")
saveRDS(MofaLoading,"/ix/djishnu/Javad/SLIDEbench_Codex/MOFA_VAE/MOFA_loading_v2.rds")
