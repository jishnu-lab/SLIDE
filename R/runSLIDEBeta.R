...
runSLIDEBeta <- function(train_y, valid_y, train_z, valid_z, method, spec, niter, fdr,
  ...
  if (!is.null(res$upsilon)) {  ## interactions found
    ...
    zs_train <- as.data.frame(train_z)[, colnames(train_z)[as.numeric(res$marginal_vars)]]
    colnames(zs_train) <- colnames(train_z)[as.numeric(res$marginal_vars)]
    zs_valid <- as.data.frame(valid_z)[, colnames(valid_z)[as.numeric(res$marginal_vars)]]
